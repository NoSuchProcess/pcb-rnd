...
void copy_strlit(const char *inds, const char *varname)
{
...
void copy_chrarr(const char *inds, const char *varname)
{
	int c, col = 16000;

	printf("/* (Using character array instead of string literal for long strings) */\n");
	printf("const char %s_arr[] = {", varname);
...
int main(int argc, char *argv[])
{
	...
	printf("%s", banner);
	if (lit)
		copy_strlit(inds, varname);
	else
		copy_chrarr(inds, varname);
	return 0;
}
