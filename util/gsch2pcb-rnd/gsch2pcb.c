...
lihata_prj:;
	...
	if (rnd_conf_load_as(RND_CFR_PROJECT, path, 0) != 0) {
		rnd_message(RND_MSG_ERROR, "Failed to parse project file %s.\n", path);
		exit(1);
	}
		...
		if (*opt == '-') {
			...
			if (!strcmp(opt, "version") || !strcmp(opt, "V")) {
				...
				if (method_find(arg) == NULL) {
					rnd_message(RND_MSG_ERROR, "Error: can't use unknown method '%s'; try --help\n", arg);
					exit(1);
				}
			...
			if (loc_str_has_suffix(argv[i], ".sch", 4) == NULL) {
				if (have_cli_project_file) {
					rnd_message(RND_MSG_ERROR, "ERROR: multiple project files specified on the command line (last one: %s). Either use multiple schematics or a single project file. Try %s --help\n", argv[i], argv[0]);
					exit(1);
				}
...
#include "gsch2pcb_rnd_conf_fields.h"
	...
	if (!have_cli_project_file && !have_cli_schematics) {
		...
		if (rnd_conf_load_as(RND_CFR_PROJECT, LOCAL_PROJECT_FILE, 0) != 0) {
			rnd_message(RND_MSG_ERROR, "Failed to load project file %s. Try %s --help\n", LOCAL_PROJECT_FILE, argv[0]);
			exit(1);
		}
		rnd_conf_update(NULL, -1); /* because of our new project file */
	}
	...
	if (!have_cli_schematics) {  /* load all schematics from the project file unless we have schematics from the cli */
		...
		if (nat != NULL) {
			...
			for (ci = rnd_conflist_first(nat->val.list); ci != NULL; ci = rnd_conflist_next(ci)) {
			...
			rnd_message(RND_MSG_ERROR, "No schematics specified on the cli or in project files. Try %s --help\n", argv[0]);
			exit(1);
		}
	}

	if (gadl_length(&schematics) == 0) {
		rnd_message(RND_MSG_ERROR, "No schematics specified on the cli; can't find any in the project files/configs either. Try %s --help\n", argv[0]);
		exit(1);
	}
	...
	if (want_method == NULL) {
		...
		if (current_method == NULL) {
			...
			if (!conf_g2pr.utils.gsch2pcb_rnd.quiet_mode)
				rnd_message(RND_MSG_WARNING, "Warning: method not specified for a project (that has no board or project file yet); defaulting to -m %s. This warning is harmless if you are running gsch2pcb-rnd for the first time on this project and you are fine with this method.", want_method);
		}
	}
	...
	if (current_method == NULL) {
		...
		if (current_method == NULL) {
			rnd_message(RND_MSG_ERROR, "Error: can't find method %s\n", want_method);
			exit(1);
		}
	}
