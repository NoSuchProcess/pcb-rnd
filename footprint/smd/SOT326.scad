...
module part_sot326(fillet=0)
{
   ...
   module fillet(pad_len=0.3, pad_width=0.3, pad_height= 0.2) {
	...
	module sot326_()
	{
		...
		rotate([90,0,-90]) scale([1.13,1.13,1.0]) translate([-0.9, 0.4, -2.0 + 0.35]) {
			// body
			color([0.1,0.1,0.1])
				linear_extrude(height=2.0)
					polygon([[-0.55,0.18],[-0.50,0.53],[0.50,0.53],[0.55,0.18],[0.45,-0.35],[-0.45,-0.35]]);

			// 6 pins
			color([0.9, 0.9, 0.9]) {
				translate([0.43,0.4,0.35-0.3/2])
					pin(0.6, 0.4, 0.3);
  				translate([0.43,0.4,1.0-0.3/2])
					pin(0.6, 0.4, 0.3);
				translate([0.43,0.4,1.65-0.3/2])
					pin(0.6, 0.4, 0.3);
