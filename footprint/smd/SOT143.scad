...
{ 
   module fillet(pad_len=0.35, pad_width=0.4, pad_height= 0.1) {
	...
	module sot143_()
	{
		...
		rotate([90,0,-90]) scale([1.13,1.13,1.0]) translate([-0.9, 0.4, -2.1 - 0.43/2]) {
			// body
			color([0.1,0.1,0.1])
				linear_extrude(height=2.9)
					polygon([[-0.6,0],[-0.45,0.53],[0.45,0.53],[0.6,0],[0.45,-0.35],[-0.45,-0.35]]);

			// 3 pins
			color([0.9, 0.9, 0.9]) {
				translate([0.35,0,0.5-0.43/2])
					pin(0.7, 0.4, 0.43);

				translate([0.35,0,2.3-0.85*3/4])
					pin(0.7, 0.4, 0.85);

				translate([-0.35,0,0.5-0.43/2])
					pin(-0.7, 0.4, 0.43);

				translate([-0.35,0,2.3-0.43/2])
					pin(-0.7, 0.4, 0.43);
