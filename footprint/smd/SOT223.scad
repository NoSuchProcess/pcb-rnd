...
{ 
   ...
   module fillet(pad_len=0.7, pad_width=0.6, pad_height= 0.2) {
	...
	module sot223_()
	{
		...
		rotate([90,0,-90]) scale([1.13,1.13,1.0]) translate([2.75, 0.75, -0.85]) {
			// body
			color([0.1,0.1,0.1])
				linear_extrude(height=6.5)
					polygon([[-1.75,0],[-1.55,0.9],[1.55,0.9],[1.75,0],[1.55,-0.7],[-1.55,-0.7]]);

			// 3 pins
			color([0.9, 0.9, 0.9]) {
				translate([-1.4,0,0.85-0.7/2])
					pin(-1.7, 0.75, 0.7);

				translate([-1.4,0,3.15-0.7/2])
					pin(-1.7, 0.75, 0.7);
				translate([-1.4,0,5.45-0.7/2])
					pin(-1.7, 0.75, 0.7);

				translate([1.4,0,3.15-3.0/2])
					pin(1.7, 0.75, 3.0);
			}
		}
	}
