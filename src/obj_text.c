...
void pcb_text_free(pcb_text_t *text)
{
	if ((text->parent.layer != NULL) && (text->parent.layer->text_tree != NULL))
		rnd_r_delete_entry(text->parent.layer->text_tree, (rnd_rnd_box_t *)text);
...
static int undo_text_geo_swap(void *udata)
{
	...
	if (layer->text_tree != NULL)
		rnd_r_delete_entry(layer->text_tree, (rnd_rnd_box_t *)g->text);
	...
	if (layer->text_tree != NULL)
		rnd_r_insert_entry(layer->text_tree, (rnd_rnd_box_t *)g->text);
...
void pcb_text_bbox(pcb_font_t *FontPtr, pcb_text_t *Text)
{
	...
	for (; s && *s; s++) {
		if (*s <= PCB_MAX_FONTPOSITION && symbol[*s].Valid) {
			...
			rnd_rnd_box_t *ds = &FontPtr->DefaultSymbol;
...
void *pcb_textop_move_buffer(pcb_opctx_t *ctx, pcb_layer_t *dstly, pcb_text_t *text)
{
	...
	rnd_r_delete_entry(srcly->text_tree, (rnd_rnd_box_t *) text);
	...
	rnd_r_insert_entry(dstly->text_tree, (rnd_rnd_box_t *) text);
...
void *pcb_textop_move_to_layer_low(pcb_opctx_t *ctx, pcb_layer_t * Source, pcb_text_t * text, pcb_layer_t * Destination)
{
	...
	rnd_r_delete_entry(Source->text_tree, (rnd_rnd_box_t *) text);
	...
	rnd_r_insert_entry(Destination->text_tree, (rnd_rnd_box_t *) text);
...
void pcb_text_pre(pcb_text_t *text)
{
	...
	if (ly->text_tree != NULL)
		rnd_r_delete_entry(ly->text_tree, (rnd_rnd_box_t *)text);
...
void pcb_text_post(pcb_text_t *text)
{
	...
	if (ly == NULL)
		return;
	if (ly->text_tree != NULL)
		rnd_r_insert_entry(ly->text_tree, (rnd_rnd_box_t *)text);
...
void pcb_text_flagchg_post(pcb_text_t *Text, unsigned long oldflagbits, void **save)
{
	...
	if (orig_layer != NULL)
		rnd_r_insert_entry(orig_layer->text_tree, (rnd_rnd_box_t *)Text);
...
{
	rnd_coord_t w = 0;
	const rnd_rnd_box_t *defaultsymbol;
	if (string == NULL)
		return 0;
...
RND_INLINE void pcb_text_draw_string_(pcb_draw_info_t *info, pcb_font_t *font, const unsigned char *string, rnd_coord_t x0, rnd_coord_t y0, double scx, double scy, double rotdeg, int mirror, rnd_coord_t thickness, rnd_coord_t min_line_width, int xordraw, rnd_coord_t xordx, rnd_coord_t xordy, pcb_text_tiny_t tiny, pcb_draw_text_cb cb, void *cb_ctx)
{
	rnd_coord_t x = 0;
	rnd_cardinal_t n;
	pcb_xform_mx_t mx = PCB_XFORM_MX_IDENT;
...
void pcb_text_draw_string(pcb_draw_info_t *info, pcb_font_t *font, const unsigned char *string, rnd_coord_t x0, rnd_coord_t y0, double scx, double scy, double rotdeg, int mirror, rnd_coord_t thickness, rnd_coord_t min_line_width, int xordraw, rnd_coord_t xordx, rnd_coord_t xordy, pcb_text_tiny_t tiny)
{
...
void pcb_text_decompose_string(pcb_draw_info_t *info, pcb_font_t *font, const unsigned char *string, rnd_coord_t x0, rnd_coord_t y0, double scx, double scy, double rotdeg, int mirror, rnd_coord_t thickness, pcb_draw_text_cb cb, void *cb_ctx)
{
