...
{
	rnd_design_t *hidlib = (rnd_design_t *)pcb;
	pcb_opctx_t ctx;
...
fgw_error_t pcb_act_LoadPixmap(fgw_arg_t *res, int argc, fgw_arg_t *argv)
{
	...
	if (fn == NULL) {
		...
		nfn = rnd_hid_fileselect(rnd_gui, "Load pixmap into buffer gfx...",
			...
			default_file, ".*", NULL, "gfx", RND_HID_FSD_READ, NULL);
...
/* returns 0 on OK */
static int pcb_actgui_ScaleBuffer(rnd_design_t *hidlib, double *x, double *y, double *th, int *recurse)
{
	scale_t ctx;
	int res;
...
/* this results in saving flipped (bottom-side) footprints whenlooking at the board from the bottom */
	PCB_SUBC_LOOP(Buffer->Data);
	{
		pcb_subc_change_side(subc, /*2 * pcb_crosshair.Y - pcb->hidlib.dwg.Y2*/0);
	}
	PCB_END_LOOP;
#endif
		...
		case F_LoadAll:
			...
			if (argc <= 2) {
				name = rnd_hid_fileselect(rnd_gui, "Load Paste Buffer ...",
					...
					default_file, ".lht", NULL, "buffer", RND_HID_FSD_READ, NULL);
