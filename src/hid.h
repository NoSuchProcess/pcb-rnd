...
struct pcb_hid_s {
	...
	int (*set_layer_group)(pcb_hid_t *hid, pcb_layergrp_id_t group, const char *purpose, int purpi, pcb_layer_id_t layer, unsigned int flags, int is_empty, pcb_xform_t **xform);
...
#define HID_SC_PAN_VIEWPORT  2
	...
	*/
	...
	void (*reg_mouse_cursor)(pcb_hid_t *hid, pcb_hidlib_t *hidlib, int idx, const char *name, const unsigned char *pixel, const unsigned char *mask);
	void (*set_mouse_cursor)(pcb_hid_t *hid, pcb_hidlib_t *hidlib, int idx);

	/* change top window title any time the after the GUI_INIT event */
	void (*set_top_title)(pcb_hid_t *hid, pcb_hidlib_t *hidlib, const char *title);
