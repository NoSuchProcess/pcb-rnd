...
static fgw_error_t pcb_act_Display(fgw_arg_t *res, int argc, fgw_arg_t *argv)
{
	...
	if (id == F_SubcID) { /* change the displayed name of subcircuits */
		if (argc > 1)
			pcb_conf_set(CFR_DESIGN, "editor/subc_id", -1, str_dir, RND_POL_OVERWRITE);
		else
			pcb_conf_set(CFR_DESIGN, "editor/subc_id", -1, "", RND_POL_OVERWRITE);
	...
	if (id == F_TermID) { /* change the displayed name of terminals */
		if (argc > 1)
			pcb_conf_set(CFR_DESIGN, "editor/term_id", -1, str_dir, RND_POL_OVERWRITE);
		else
			pcb_conf_set(CFR_DESIGN, "editor/term_id", -1, "", RND_POL_OVERWRITE);
	...
	if (!str_dir || !*str_dir) {
		...
		case F_CycleCrosshair:
			...
			if (pcb_ch_shape_NUM == pcbhl_conf.editor.crosshair_shape_idx)
				pcb_conf_set(CFR_CLI, "editor/crosshair_shape_idx", 0, "0", RND_POL_OVERWRITE);
			pcb_hid_notify_crosshair_change(RND_ACT_HIDLIB, pcb_true);
			break;
...
static fgw_error_t pcb_act_ClipInhibit(fgw_arg_t *res, int argc, fgw_arg_t *argv)
{
	...
	tmp[1] = '\0';
	pcb_conf_set(CFR_CLI, "temp/clip_inhibit_chg", 0, tmp, RND_POL_OVERWRITE);
