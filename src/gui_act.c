...
static fgw_error_t pcb_act_Display(fgw_arg_t *res, int argc, fgw_arg_t *argv)
{
	...
	if (id == F_SubcID) { /* change the displayed name of subcircuits */
		...
		pcb_gui->invalidate_all(pcb_gui, &PCB->hidlib); /* doesn't change too often, isn't worth anything more complicated */
		pcb_draw();
		return 0;
	}
...
static fgw_error_t pcb_act_Mode(fgw_arg_t *res, int argc, fgw_arg_t *argv)
{
	...
	case F_None:
		...
		break;
	case F_Copy:
		pcb_tool_select_by_id(&PCB->hidlib, PCB_MODE_COPY);
		break;
	case F_InsertPoint:
		pcb_tool_select_by_id(&PCB->hidlib, PCB_MODE_INSERT_POINT);
		break;
	case F_Line:
		pcb_tool_select_by_id(&PCB->hidlib, PCB_MODE_LINE);
		break;
	case F_Lock:
		pcb_tool_select_by_id(&PCB->hidlib, PCB_MODE_LOCK);
		break;
	case F_Move:
		pcb_tool_select_by_id(&PCB->hidlib, PCB_MODE_MOVE);
		break;
	case F_Cancel:
		{
	...
	case F_Rectangle:
		...
		break;
	case F_Rotate:
		pcb_tool_select_by_id(&PCB->hidlib, PCB_MODE_ROTATE);
		break;
	case F_Stroke:
		...
		break;
	case F_Text:
		pcb_tool_select_by_id(&PCB->hidlib, PCB_MODE_TEXT);
		break;
	case F_Thermal:
		pcb_tool_select_by_id(&PCB->hidlib, PCB_MODE_THERMAL);
		break;
	case F_Via:
		pcb_tool_select_by_id(&PCB->hidlib, PCB_MODE_VIA);
		break;
...
static fgw_error_t pcb_act_Cursor(fgw_arg_t *res, int argc, fgw_arg_t *argv)
{
	...
	pcb_gui->view_get(pcb_gui, &PCB->hidlib, &vbx);
