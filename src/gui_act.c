...
static fgw_error_t pcb_act_Display(fgw_arg_t *res, int argc, fgw_arg_t *argv)
{
	...
	if (!str_dir || !*str_dir) {
		...
		case F_ToggleAllDirections:
			...
			pcb_tool_adjust_attached_objects(argv[0].val.argv0.user_call_ctx);
			break;
		...
		case F_Pinout:
			return pcb_actionva(argv[0].val.argv0.user_call_ctx, "pinout", NULL);
...
static fgw_error_t pcb_act_Mode(fgw_arg_t *res, int argc, fgw_arg_t *argv)
{
	...
	case F_None:
		...
		break;
	case F_Copy:
		pcb_tool_select_by_id(argv[0].val.argv0.user_call_ctx, PCB_MODE_COPY);
		break;
	case F_InsertPoint:
		pcb_tool_select_by_id(argv[0].val.argv0.user_call_ctx, PCB_MODE_INSERT_POINT);
		break;
	case F_Line:
		pcb_tool_select_by_id(argv[0].val.argv0.user_call_ctx, PCB_MODE_LINE);
		break;
	case F_Lock:
		pcb_tool_select_by_id(argv[0].val.argv0.user_call_ctx, PCB_MODE_LOCK);
		break;
	case F_Move:
		pcb_tool_select_by_id(argv[0].val.argv0.user_call_ctx, PCB_MODE_MOVE);
		break;
	case F_Cancel:
		{
			int saved_mode = pcbhl_conf.editor.mode;
			pcb_tool_select_by_id(argv[0].val.argv0.user_call_ctx, saved_mode);
		}
		break;
	case F_Escape:
		{
			...
			case PCB_MODE_RECTANGLE:
				if (pcb_crosshair.AttachedBox.State == PCB_CH_STATE_FIRST)
					...
					pcb_tool_select_by_id(argv[0].val.argv0.user_call_ctx, PCB_MODE_RECTANGLE);
				}
				break;
			...
			case PCB_MODE_POLYGON:
				if (pcb_crosshair.AttachedLine.State == PCB_CH_STATE_FIRST)
					...
					pcb_tool_select_by_id(argv[0].val.argv0.user_call_ctx, PCB_MODE_POLYGON);
				}
				break;
			...
			case PCB_MODE_POLYGON_HOLE:
				if (pcb_crosshair.AttachedLine.State == PCB_CH_STATE_FIRST)
					...
					pcb_tool_select_by_id(argv[0].val.argv0.user_call_ctx, PCB_MODE_POLYGON_HOLE);
				}
				break;
	...
	case F_Notify:
		pcb_notify_mode(argv[0].val.argv0.user_call_ctx);
		break;
	case F_PasteBuffer:
		pcb_tool_select_by_id(argv[0].val.argv0.user_call_ctx, PCB_MODE_PASTE_BUFFER);
		break;
	case F_Polygon:
		pcb_tool_select_by_id(argv[0].val.argv0.user_call_ctx, PCB_MODE_POLYGON);
		break;
	...
	case F_Release:
		...
		pcb_release_mode(argv[0].val.argv0.user_call_ctx);
		break;
	case F_Remove:
		pcb_tool_select_by_id(argv[0].val.argv0.user_call_ctx, PCB_MODE_REMOVE);
		break;
	case F_Rectangle:
		pcb_tool_select_by_id(argv[0].val.argv0.user_call_ctx, PCB_MODE_RECTANGLE);
		break;
	case F_Rotate:
		pcb_tool_select_by_id(argv[0].val.argv0.user_call_ctx, PCB_MODE_ROTATE);
		break;
	case F_Stroke:
		...
		if (pcbhl_conf.editor.mode == PCB_MODE_LINE && pcb_crosshair.AttachedLine.State != PCB_CH_STATE_FIRST)
			...
			pcb_tool_save(argv[0].val.argv0.user_call_ctx);
		...
		break;
	case F_Text:
		pcb_tool_select_by_id(argv[0].val.argv0.user_call_ctx, PCB_MODE_TEXT);
		break;
	case F_Thermal:
		pcb_tool_select_by_id(argv[0].val.argv0.user_call_ctx, PCB_MODE_THERMAL);
		break;
	case F_Via:
		pcb_tool_select_by_id(argv[0].val.argv0.user_call_ctx, PCB_MODE_VIA);
		break;

	case F_Restore: /* restore the last saved tool */
		pcb_tool_restore(argv[0].val.argv0.user_call_ctx);
		break;
...
/* set layer current and size from line or arc */
	...
	case PCB_OBJ_LINE:
		...
		if (pcbhl_conf.editor.mode != PCB_MODE_LINE)
			pcb_tool_select_by_id(argv[0].val.argv0.user_call_ctx, PCB_MODE_LINE);
		pcb_notify_crosshair_change(pcb_true);
		pcb_event(argv[0].val.argv0.user_call_ctx, PCB_EVENT_ROUTE_STYLES_CHANGED, NULL);
		break;
...
static fgw_error_t pcb_act_EditLayer(fgw_arg_t *res, int argc, fgw_arg_t *argv)
{
	...
	if (interactive) {
		...
		args[0].val.argv0.user_call_ctx = &PCB->hidlib;
