...
void pcb_arc_set_angles(pcb_layer_t *Layer, pcb_arc_t *a, rnd_angle_t new_sa, rnd_angle_t new_da)
{
	...
	rnd_rtree_delete(Layer->arc_tree, a, (rnd_rtree_box_t *)a);
	pcb_undo_add_obj_to_change_angles(PCB_OBJ_ARC, a, a, a);
	a->StartAngle = new_sa;
	a->Delta = new_da;
	pcb_arc_bbox(a);
	rnd_rtree_insert(Layer->arc_tree, a, (rnd_rtree_box_t *)a);
...
void pcb_arc_free(pcb_arc_t *arc)
{
	if ((arc->parent.layer != NULL) && (arc->parent.layer->arc_tree != NULL))
		rnd_rtree_delete(arc->parent.layer->arc_tree, arc, (rnd_rtree_box_t *)arc);
...
void pcb_arc_pre(pcb_arc_t *arc)
{
	...
	if (ly == NULL)
		return;
	if (ly->arc_tree != NULL)
		rnd_rtree_delete(ly->arc_tree, arc, (rnd_rtree_box_t *)arc);
...
void pcb_arc_post(pcb_arc_t *arc)
{
	...
	if (ly == NULL)
		return;
	if (ly->arc_tree != NULL)
		rnd_rtree_insert(ly->arc_tree, arc, (rnd_rtree_box_t *)arc);
...
static int undo_arc_geo_swap(void *udata)
{
	...
	if (layer->arc_tree != NULL)
		rnd_rtree_delete(layer->arc_tree, g->arc, (rnd_rtree_box_t *)g->arc);
	...
	pcb_arc_bbox(g->arc);
	if (layer->arc_tree != NULL)
		rnd_rtree_insert(layer->arc_tree, g->arc, (rnd_rtree_box_t *)g->arc);
...
void *pcb_arcop_move_buffer(pcb_opctx_t *ctx, pcb_layer_t *dstly, pcb_arc_t *arc)
{
	...
	rnd_rtree_delete(srcly->arc_tree, arc, (rnd_rtree_box_t *)arc);
...
{
	rnd_rtree_delete(Source->arc_tree, arc, (rnd_rtree_box_t *)arc);
