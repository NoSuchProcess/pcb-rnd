...
static void BuildObjectList(vtid_t objs[2], pcb_any_obj_t *obj)
{
	switch(obj->type) {
	case PCB_OBJ_LINE:
	case PCB_OBJ_ARC:
	...
	default:
		fprintf(stderr, "Internal error in BuildObjectList: unknown object type %i\n", thing_type);
	}
}
...
doIsBad:
	...
	drcerr_count++;
	LocateError(&x, &y);
	BuildObjectList(objs, (pcb_any_obj_t *)ptr2);
...
static int drc_text(pcb_drc_list_t *lst, pcb_layer_t *layer, pcb_text_t *text, pcb_coord_t min_wid, pcb_coord_t *x, pcb_coord_t *y)
{
	...
	if (text->thickness < min_wid) {
		...
		drcerr_count++;
		LocateError(x, y);
		BuildObjectList(objs, (pcb_any_obj_t *)text);
	...
	if (!IsBad) {
		...
		PCB_LINE_COPPER_LOOP(PCB->Data);
		{
			...
			if (line->Thickness < conf_core.design.min_wid) {
				...
				drcerr_count++;
				LocateError(&x, &y);
				BuildObjectList(objs, (pcb_any_obj_t *)line);
				violation = pcb_drc_violation_new(
	...
	if (!IsBad) {
		PCB_ARC_COPPER_LOOP(PCB->Data);
		{
			...
			if (arc->Thickness < conf_core.design.min_wid) {
				...
				drcerr_count++;
				LocateError(&x, &y);
				BuildObjectList(objs, (pcb_any_obj_t *)arc);
				violation = pcb_drc_violation_new(
		...
		PCB_PADSTACK_LOOP(PCB->Data);
		{
			...
			if ((ring > 0) || (hole > 0)) {
				...
				if (ring) {
					drcerr_count++;
					LocateError(&x, &y);
					BuildObjectList(objs, (pcb_any_obj_t *)padstack);
					violation = pcb_drc_violation_new(
...
static pcb_bool DRCFind(pcb_drc_list_t *lst, int What, void *ptr1, void *ptr2, void *ptr3)
{
	...
	if (conf_core.design.shrink != 0) {
		...
		if (DoIt(pcb_true, pcb_false)) {
			...
			drcerr_count++;
			LocateError(&x, &y);
			BuildObjectList(objs, (pcb_any_obj_t *)thing_ptr2);
			violation = pcb_drc_violation_new(
