...
void pcb_line_draw_(pcb_line_t *line, int allow_term_gfx)
{
	...
	if (!conf_core.editor.thin_draw) {
		if ((line->term != NULL) && (allow_term_gfx) && !PCB_FLAG_TEST(PCB_FLAG_FOUND, line) && !PCB_FLAG_TEST(PCB_FLAG_WARN, line)) {
			pcb_hid_gc_t gc = PCB_FLAG_TEST(PCB_FLAG_SELECTED, line) ? Output.padselGC : Output.active_padGC;
			pcb_gui->set_line_width(gc, line->Thickness);
