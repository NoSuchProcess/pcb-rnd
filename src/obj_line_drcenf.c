...
static rnd_rtree_dir_t drcPstk_callback(void *cl, void *obj, const rnd_rtree_box_t *box)
{
...
static rnd_rtree_dir_t drcLine_callback(void *cl, void *obj, const rnd_rtree_box_t *box)
{
...
static rnd_rtree_dir_t drcArc_callback(void *cl, void *obj, const rnd_rtree_box_t *box)
{
	...
	while (length != last) {
		...
		while (length2 != last2) {
			...
			if (setjmp(info.env) == 0) {
				info.line = &line1;
				rnd_rtree_search_any(PCB->Data->padstack_tree, (rnd_rtree_box_t*)&line1.BoundingBox, NULL, drcPstk_callback, &info, NULL);
				if (two_lines) {
					info.line = &line2;
					rnd_rtree_search_any(PCB->Data->padstack_tree, (rnd_rtree_box_t*)&line2.BoundingBox, NULL, drcPstk_callback, &info, NULL);
				}
				PCB_COPPER_GROUP_LOOP(PCB->Data, group);
				{
					info.line = &line1;
					rnd_rtree_search_any(layer->line_tree, (rnd_rtree_box_t*)&line1.BoundingBox, NULL, drcLine_callback, &info, NULL);
					rnd_rtree_search_any(layer->arc_tree, (rnd_rtree_box_t*)&line1.BoundingBox, NULL, drcArc_callback, &info, NULL);
					if (two_lines) {
						info.line = &line2;
						rnd_rtree_search_any(layer->line_tree, (rnd_rtree_box_t*)&line2.BoundingBox, NULL, drcLine_callback, &info, NULL);
						rnd_rtree_search_any(layer->arc_tree, (rnd_rtree_box_t*)&line2.BoundingBox, NULL, drcArc_callback, &info, NULL);
					}
				}
				PCB_END_LOOP;
				/* no intersector! */
...
static void drc_line(rnd_point_t *end)
{
	...
	if (setjmp(info.env) == 0) {
		info.line = &line;
		rnd_rtree_search_any(PCB->Data->padstack_tree, (rnd_rtree_box_t *)&line.BoundingBox, NULL, drcPstk_callback, &info, NULL);
		PCB_COPPER_GROUP_LOOP(PCB->Data, group);
		{
			info.line = &line;
			rnd_rtree_search_any(layer->line_tree, (rnd_rtree_box_t *)&line.BoundingBox, NULL, drcLine_callback, &info, NULL);
			rnd_rtree_search_any(layer->arc_tree, (rnd_rtree_box_t *)&line.BoundingBox, NULL, drcArc_callback, &info, NULL);
		}
		PCB_END_LOOP;
		/* no intersector! */
		auto_good:;
