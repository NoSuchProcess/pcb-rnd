...
static pcb_bool ADD_PS_TO_LIST(pcb_pstk_t *ps, int from_type, void *from_ptr, pcb_found_conn_type_t type)
{
	...
	if (drc && !PCB_FLAG_TEST(PCB_FLAG_SELECTED, ps))
		return (SetThing(PCB_OBJ_PSTK, pcb_obj_parent_subc((pcb_any_obj_t *)ps), ps, ps));
	return pcb_false;
}
...
static pcb_bool ADD_PADSTACK_TO_LIST(pcb_pstk_t *ps, int from_type, void *from_ptr, pcb_found_conn_type_t type)
{
	...
	if (drc && !PCB_FLAG_TEST(PCB_FLAG_SELECTED, ps) && (ps->parent.data->parent_type == PCB_PARENT_SUBC))
		return (SetThing(PCB_OBJ_PSTK, ps->parent.data->parent.subc, ps, ps));
	return pcb_false;
}
...
static pcb_bool ADD_LINE_TO_LIST(pcb_layer_id_t L, pcb_line_t *Ptr, int from_type, void *from_ptr, pcb_found_conn_type_t type)
{
	...
	if (drc && !PCB_FLAG_TEST(PCB_FLAG_SELECTED, (Ptr)))
		return (SetThing(PCB_OBJ_LINE, LAYER_PTR(L), (Ptr), (Ptr)));
	return pcb_false;
}
...
#endif
	if (drc && !PCB_FLAG_TEST(PCB_FLAG_SELECTED, (Ptr)))
		return (SetThing(PCB_OBJ_ARC, LAYER_PTR(L), (Ptr), (Ptr)));
	return pcb_false;
}
...
#endif
	if (drc && !PCB_FLAG_TEST(PCB_FLAG_SELECTED, (Ptr)))
		return (SetThing(PCB_OBJ_RAT, (Ptr), (Ptr), (Ptr)));
	return pcb_false;
}
...
#endif
	if (drc && !PCB_FLAG_TEST(PCB_FLAG_SELECTED, (Ptr)))
		return (SetThing(PCB_OBJ_POLY, LAYER_PTR(L), (Ptr), (Ptr)));
	return pcb_false;
}
...
#define SKIP_IF_CUT(o1, o2, action) \
	if (((o1)->intcut) || ((o2)->intcut)) { \
		if (pcb_obj_parent_subc((pcb_any_obj_t *)o1) == pcb_obj_parent_subc((pcb_any_obj_t *)o2)) { \
			action; \
		} \
	} \
...
static pcb_r_dir_t LOCtoPSline_callback(const pcb_box_t * b, void *cl)
{
	...
	SKIP_IF_CUT(&i->ps, line, return PCB_R_DIR_NOT_FOUND);
...
static pcb_r_dir_t LOCtoPSarc_callback(const pcb_box_t * b, void *cl)
{
	...
	SKIP_IF_CUT(&i->ps, arc, return PCB_R_DIR_NOT_FOUND);
...
static pcb_r_dir_t LOCtoPSpoly_callback(const pcb_box_t * b, void *cl)
{
	...
	SKIP_IF_CUT(&i->ps, polygon, return PCB_R_DIR_NOT_FOUND);
...
static pcb_r_dir_t PStoPS_callback(const pcb_box_t *b, void *cl)
{
	...
	SKIP_IF_CUT(&i->ps, ps2, return PCB_R_DIR_NOT_FOUND);
...
static pcb_r_dir_t ps_line_callback(const pcb_box_t * b, void *cl)
{
	...
	SKIP_IF_CUT(ps, &i->line, return PCB_R_DIR_NOT_FOUND);
...
static pcb_r_dir_t ps_arc_callback(const pcb_box_t * b, void *cl)
{
	...
	SKIP_IF_CUT(ps, &i->arc, return PCB_R_DIR_NOT_FOUND);
...
static pcb_r_dir_t ps_poly_callback(const pcb_box_t * b, void *cl)
{
	...
	SKIP_IF_CUT(ps, &i->polygon, return PCB_R_DIR_NOT_FOUND);
...
static pcb_r_dir_t LOCtoArcLine_callback(const pcb_box_t * b, void *cl)
{
	...
	SKIP_IF_CUT(line, &i->arc, return PCB_R_DIR_NOT_FOUND);
...
static pcb_r_dir_t LOCtoArcArc_callback(const pcb_box_t * b, void *cl)
{
	...
	SKIP_IF_CUT(&i->arc, arc, return PCB_R_DIR_NOT_FOUND);
	...
	for (entry = 0; entry < PCB->LayerGroups.grp[LayerGroup].len; entry++) {
		...
		/* now check all polygons */
		{
			...
			for(b = pcb_r_first(PCB->Data->Layer[layer].polygon_tree, &it); b != NULL; b = pcb_r_next(&it)) {
				pcb_poly_t *polygon = (pcb_poly_t *)b;
				SKIP_IF_CUT(polygon, Arc, continue);
...
static pcb_r_dir_t LOCtoLineLine_callback(const pcb_box_t * b, void *cl)
{
	...
	SKIP_IF_CUT(&i->line, line, return PCB_R_DIR_NOT_FOUND);
...
static pcb_r_dir_t LOCtoLineArc_callback(const pcb_box_t * b, void *cl)
{
	...
	SKIP_IF_CUT(&i->line, arc, return PCB_R_DIR_NOT_FOUND);
...
static pcb_bool LookupLOConnectionsToLine(pcb_line_t *Line, pcb_cardinal_t LayerGroup, pcb_bool PolysTo)
{
	...
	for (entry = 0; entry < PCB->LayerGroups.grp[LayerGroup].len; entry++) {
		...
		if (PolysTo) {
			...
			for(b = pcb_r_first(PCB->Data->Layer[layer].polygon_tree, &it); b != NULL; b = pcb_r_next(&it)) {
				pcb_poly_t *polygon = (pcb_poly_t *)b;
				SKIP_IF_CUT(polygon, Line, continue);
...
static pcb_r_dir_t LOCtoPolyLine_callback(const pcb_box_t * b, void *cl)
{
	...
	SKIP_IF_CUT(&i->polygon, line, return PCB_R_DIR_NOT_FOUND);
...
static pcb_r_dir_t LOCtoPolyArc_callback(const pcb_box_t * b, void *cl)
{
	...
	SKIP_IF_CUT(&i->polygon, arc, return PCB_R_DIR_NOT_FOUND);
...
/* loop over all layers of the group */
	for (entry = 0; entry < PCB->LayerGroups.grp[LayerGroup].len; entry++) {
		...
		/* check all polygons */
		{
			...
			for(b = pcb_r_first(PCB->Data->Layer[layer].polygon_tree, &it); b != NULL; b = pcb_r_next(&it)) {
				pcb_poly_t *polygon = (pcb_poly_t *)b;
				SKIP_IF_CUT(polygon, Polygon, continue);
