...
void ghid_main_menu_real_add_resource(GHidMainMenu * menu, GtkMenuShell * shell, const Resource * res, const char *path);


static GHashTable *menu_hash = NULL;	/* path->GtkWidget */

static GtkAction *ghid_add_menu(GHidMainMenu * menu, GtkMenuShell * shell, const Resource * sub_res, const char *path)
{
	const char *res_val;
	...
	if (sub_res->flags & FLAG_S) {
		...
		ghid_main_menu_real_add_resource(menu, GTK_MENU_SHELL(submenu), sub_res, npath);
	...
	if (action) {
		...
		g_signal_connect(G_OBJECT(action), "activate", menu->action_cb, (gpointer) sub_res);
	...
	for (i = 0; i < res->c; ++i) {
			...
			for (j = 0; j < sub_res->c; j++) {
				...
				if (resource_type(sub_res->v[j]) == 110) {
					...
					if (strcmp(res_name, "m") == 0
							|| strcmp(res_name, "a") == 0 || strcmp(res_name, "sensitive") == 0 || strcmp(res_name, "tip") == 0)
						break;
		...
		case 10:										/* no name, value */
			...
			if (res->v[i].value[0] == '@') {
				...
				GtkWidget *item = gtk_menu_item_new_with_label(res->v[i].value);
				gtk_menu_shell_append(shell, item);
			}
			break;
		}
																	 ...
																	 void (*cb) (GtkAction *, const char *toggle_flag, const char *active_flag))
{
	...
	for (list = menu->actions; list; list = list->next) {
		...
		const char *af = g_object_get_data(G_OBJECT(list->data),
																			 "active-flag");
		g_signal_handlers_block_by_func(G_OBJECT(list->data), menu->action_cb, res);
		cb(GTK_ACTION(list->data), tf, af);
		g_signal_handlers_unblock_by_func(G_OBJECT(list->data), menu->action_cb, res);
	}
}
