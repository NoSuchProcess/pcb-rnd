...
static void pcb_draw_delayed_objs(pcb_draw_info_t *info)
{
	...
	for(n = 0; n < delayed_objs.used; n++) {
		...
		switch(o->type) {
			case PCB_OBJ_ARC:  pcb_arc_draw_term_callback(info, (void *)b, b); break;
			case PCB_OBJ_LINE: pcb_line_draw_term_callback(info, (void *)b, b); break;
			case PCB_OBJ_TEXT: pcb_text_draw_term_callback(info, (void *)b, b); break;
...
void pcb_draw_layer(pcb_draw_info_t *info, const pcb_layer_t *Layer_)
{
	...
	if (lflg & PCB_LYT_COPPER) {
		...
		rnd_rtree_search_any(Layer->gfx_tree, (rnd_rtree_box_t *)info->drawn_area, NULL, pcb_gfx_draw_under_callback, info, NULL);
	...
	if (lflg & PCB_LYT_COPPER) {
		delayed_terms_enabled = rnd_true;
		rnd_rtree_search_any(Layer->line_tree, (rnd_rtree_box_t *)info->drawn_area, NULL, pcb_line_draw_term_callback, info, NULL);
		rnd_rtree_search_any(Layer->arc_tree, (rnd_rtree_box_t *)info->drawn_area, NULL, pcb_arc_draw_term_callback, info, NULL);
		rnd_rtree_search_any(Layer->text_tree, (rnd_rtree_box_t *)info->drawn_area, NULL, pcb_text_draw_term_callback, info, NULL);
		rnd_rtree_search_any(Layer->gfx_tree, (rnd_rtree_box_t *)info->drawn_area, NULL, pcb_gfx_draw_above_callback, info, NULL);
		delayed_terms_enabled = rnd_false;
		may_have_delayed = 1;
	}
	else {
		rnd_rtree_search_any(Layer->line_tree, (rnd_rtree_box_t *)info->drawn_area, NULL, pcb_line_draw_callback, info, NULL);
		rnd_rtree_search_any(Layer->arc_tree, (rnd_rtree_box_t *)info->drawn_area, NULL, pcb_arc_draw_callback, info, NULL);
		rnd_rtree_search_any(Layer->text_tree, (rnd_rtree_box_t *)info->drawn_area, NULL, pcb_text_draw_callback, info, NULL);
		rnd_rtree_search_any(Layer->gfx_tree, (rnd_rtree_box_t *)info->drawn_area, NULL, pcb_gfx_draw_above_callback, info, NULL);
	}
...
void pcb_draw_layer_under(pcb_board_t *pcb, const pcb_layer_t *Layer, const rnd_box_t *screen, pcb_data_t *data, rnd_xform_t *xf)
{
		...
		if (Layer->gfx_tree != NULL)
			for(o = rnd_rtree_first(&it, Layer->gfx_tree, (rnd_rtree_box_t *)screen); o != NULL; o = rnd_rtree_next(&it))
				if (pcb_obj_is_under(o, data))
					pcb_gfx_draw_under_callback(&info, (void *)o, (rnd_rtree_box_t *)o);
	}
	...
	if (lflg & PCB_LYT_COPPER) {
		...
		if (Layer->line_tree != NULL)
			for(o = rnd_rtree_first(&it, Layer->line_tree, (rnd_rtree_box_t *)screen); o != NULL; o = rnd_rtree_next(&it))
				if (pcb_obj_is_under(o, data))
					pcb_line_draw_callback(&info, (void *)o, (rnd_rtree_box_t *)o);
		if (Layer->arc_tree != NULL)
			for(o = rnd_rtree_first(&it, Layer->arc_tree, (rnd_rtree_box_t *)screen); o != NULL; o = rnd_rtree_next(&it))
				if (pcb_obj_is_under(o, data))
					pcb_arc_draw_callback(&info, (void *)o, (rnd_rtree_box_t *)o);
		if (Layer->text_tree != NULL)
			for(o = rnd_rtree_first(&it, Layer->text_tree, (rnd_rtree_box_t *)screen); o != NULL; o = rnd_rtree_next(&it))
				if (pcb_obj_is_under(o, data))
					pcb_text_draw_callback(&info, (void *)o, (rnd_rtree_box_t *)o);
		if (Layer->gfx_tree != NULL)
			for(o = rnd_rtree_first(&it, Layer->gfx_tree, (rnd_rtree_box_t *)screen); o != NULL; o = rnd_rtree_next(&it))
				if (pcb_obj_is_under(o, data))
					pcb_gfx_draw_above_callback(&info, (void *)o, (rnd_rtree_box_t *)o);
	}
...
#define PCB_TERM_LABEL_SETUP(label) \
	...
	if (vert) { \
		h = rnd_font_string_width(font, 0, scl, label); \
		w = rnd_font_string_height(font, 0, scl, label); \
	} \
	else { \
		w = rnd_font_string_width(font, 0, scl, label); \
		h = rnd_font_string_height(font, 0, scl, label); \
	} \
	dx = w / 2; \
	dy = h / 2; \
	if (flip_x ^ flip_y) { \
...
void pcb_label_draw(pcb_draw_info_t *info, rnd_coord_t x, rnd_coord_t y, double scale, rnd_bool vert, rnd_bool centered, const char *label, int prio)
{
	...
	if (!conf_core.appearance.smart_labels) {
		...
		pcb_text_draw_string(info, font, (unsigned const char *)label, x, y, scale/100, scale/100, direction*90.0, mirror, conf_core.appearance.label_thickness, 0, 0, 0, 0, PCB_TXT_TINY_HIDE, 0);
		if (rnd_render->gui)
			pcb_draw_force_termlab--;
	}
	else
