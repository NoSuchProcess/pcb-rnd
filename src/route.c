		...
		switch (p_obj->type) {
			case PCB_OBJ_LINE:
				...
				if (apply_to_line) {
					...
					pcb_r_insert_entry(layer->line_tree, (rnd_box_t *) apply_to_line);
			...
			case PCB_OBJ_ARC:
				...
				if (apply_to_arc) {
					...
					if (changes > 0) {
						...
						pcb_r_delete_entry(apply_to_arc_layer->arc_tree, (rnd_box_t *) apply_to_arc);
						...
						pcb_r_insert_entry(apply_to_arc_layer->arc_tree, (rnd_box_t *) apply_to_arc);
