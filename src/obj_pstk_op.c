...
void *pcb_pstkop_move_buffer(pcb_opctx_t *ctx, pcb_pstk_t *ps)
{
	...
	pcb_r_delete_entry(ctx->buffer.src->padstack_tree, (rnd_box_t *)ps);
	...
	pcb_r_insert_entry(ctx->buffer.dst->padstack_tree, (rnd_box_t *)ps);
...
void *pcb_pstkop_move(pcb_opctx_t *ctx, pcb_pstk_t *ps)
{
	...
	pcb_r_delete_entry(data->padstack_tree, (rnd_box_t *)ps);
	pcb_poly_restore_to_poly(data, PCB_OBJ_PSTK, NULL, ps);
	pcb_pstkop_move_noclip(ctx, ps);
	pcb_r_insert_entry(data->padstack_tree, (rnd_box_t *)ps);
...
{
	pcb_r_delete_entry(ctx->remove.destroy_target->padstack_tree, (rnd_box_t *)ps);
	pcb_pstk_free(ps);
	return NULL;
}
...
void *pcb_pstkop_rotate(pcb_opctx_t *ctx, pcb_pstk_t *ps)
{
	...
	if (pcb_pstk_change_instance(ps, NULL, NULL, &rot, NULL, NULL) == 0) {
		...
		pcb_pstk_bbox(ps);
		if (ps->parent.data->padstack_tree != NULL)
			pcb_r_insert_entry(ps->parent.data->padstack_tree, (rnd_box_t *)ps);
