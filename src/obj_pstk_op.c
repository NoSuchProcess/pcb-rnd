...
void *pcb_pstkop_move_buffer(pcb_opctx_t *ctx, pcb_pstk_t *ps)
{
	...
	rnd_rtree_delete(ctx->buffer.src->padstack_tree, ps, (rnd_rtree_box_t *)ps);
...
void *pcb_pstkop_move(pcb_opctx_t *ctx, pcb_pstk_t *ps)
{
	...
	rnd_rtree_delete(data->padstack_tree, ps, (rnd_rtree_box_t *)ps);
...
{
	rnd_rtree_delete(ctx->remove.destroy_target->padstack_tree, ps, (rnd_rtree_box_t *)ps);
	pcb_pstk_free(ps);
	return NULL;
}
