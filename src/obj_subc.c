		...
		if ((conf_core.editor.subc_id != NULL) && (*conf_core.editor.subc_id != '\0')) {
			...
			if (pcb_append_dyntext(&s, (pcb_any_obj_t *)subc, conf_core.editor.subc_id) == 0) {
				...
				for(curr = s.array; curr != NULL; curr = next) {
					...
					pcb_term_label_draw(x, y, conf_core.appearance.term_label_size/2, 0, 0, curr, subc->intconn);
					if (ctrl) {
						switch(*next) {
				...
				pcb_term_label_draw(x0, y0, conf_core.appearance.term_label_size/2.0, 0, 0, "<err>", subc->intconn);
		}
		else
			pcb_term_label_draw(x0, y0, conf_core.appearance.term_label_size/2.0, 0, 0, subc->refdes, subc->intconn);
	}
	return PCB_R_DIR_FOUND_CONTINUE;
}
