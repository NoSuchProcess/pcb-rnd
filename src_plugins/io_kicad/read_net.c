...
#define if_strval(node, name) \
	if (strcmp(node->str, #name) == 0) { \
		if (name != NULL) { \
			rnd_message(RND_MSG_ERROR, "Invalid eeschema: multiple %s subtrees\n", #name); \
			return -1; \
		} \
...
#define req_subtree(name) \
	if (name == NULL) { \
		rnd_message(RND_MSG_ERROR, "Invalid eeschema: missing %s subtree\n", #name); \
		return -1; \
	} \
...
static int eeschema_parse_net(gsxl_dom_t *dom)
{
	...
	if ((version->children == NULL) || (strcmp(version->children->str, "D") != 0)) {
		rnd_message(RND_MSG_ERROR, "Invalid eeschema version: expected 'D', got '%s'\n", version->children->str);
		return -1;
	}
	...
	for(c = components->children; c != NULL; c = c->next) {
		...
		if (footprint == NULL) {
			rnd_message(RND_MSG_WARNING, "eeschema: ignoring component %s with no footprint\n", ref);
			continue;
		}
...
fgw_error_t pcb_act_LoadeeschemaFrom(fgw_arg_t *res, int argc, fgw_arg_t *argv)
{
	...
	RND_ACT_MAY_CONVARG(1, FGW_STR, LoadeeschemaFrom, fname = argv[1].val.str);

	if (!fname || !*fname) {
		fname = rnd_hid_fileselect(rnd_gui, "Load eeschema netlist file...",
...
void pcb_eeschema_init(void)
{
	...
	import_eeschema.desc             = "schematics from KiCad's eeschema";
