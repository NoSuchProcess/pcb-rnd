		...
		if (es && strcmp(es, ename) == 0) {
			PCB_PIN_LOOP(element);
			{
				if (strcmp(pin->Number, pname) == 0) {
					rnd_hidcore_crosshair_move_to(ltf_hidlib, pin->X, pin->Y, 0);
					free(ename);
					return;
				}
			}
			...
			PCB_PAD_LOOP(element);
			{
				if (strcmp(pad->Number, pname) == 0) {
					rnd_coord_t x = (pad->Point1.X + pad->Point2.X) / 2;
					rnd_coord_t y = (pad->Point1.Y + pad->Point2.Y) / 2;
...
{
	RND_REGISTER_ACTIONS(ltf_netlist_action_list, lesstif_cookie);
}
