...
static void text_at(pcb_draw_info_t *info, rnd_hid_gc_t gc, int x, int y, int align, const char *fmt, ...)
{
	char tmp[512];
	int w = 0, i;
	pcb_text_t t;
	...
	t.Y = y;
	for (i = 0; tmp[i]; i++)
		w += (font->glyph[(int)tmp[i]].width + font->glyph[(int)tmp[i]].xdelta);
	...
	if (align & 8)
		fab_line(gc, t.X,
						 t.Y + PCB_SCALE_TEXT(font->max_height, t.Scale) + RND_MIL_TO_COORD(10),
						 t.X + w, t.Y + PCB_SCALE_TEXT(font->max_height, t.Scale) + RND_MIL_TO_COORD(10));
}

/* Y, +, X, circle, square */
static void drill_sym(rnd_hid_gc_t gc, int idx, int x, int y)
{
	int type = idx % 5;
	int size = idx / 5;
...
static void DrawFab(pcb_draw_info_t *info, rnd_hid_gc_t gc, const rnd_hid_expose_ctx_t *e)
{
	...
	yoff = -TEXT_LINE;
	for (found = 0, i = 0; i < pcb_max_layer(PCB); i++) {
...
{
	rnd_conf_unreg_fields("plugins/draw_fab/");
}
...
#define conf_reg(field,isarray,type_name,cpath,cname,desc,flags) \
	rnd_conf_reg_field(conf_draw_fab, field,isarray,type_name,cpath,cname,desc,flags);
