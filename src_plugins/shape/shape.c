...
static pcb_poly_t *any_poly_place(pcb_data_t *data, pcb_layer_t *layer, pcb_poly_t *p)
{
	...
	if (data != PCB->Data) {
		...
		pcb_r_delete_entry(PCB_CURRLAYER(PCB)->polygon_tree, (rnd_box_t *)p);
		pcb_poly_free_fields(p);
		pcb_poly_free(p);
...
static pcb_line_t *circle_place(pcb_data_t *data, pcb_layer_t *layer, rnd_coord_t dia, rnd_coord_t cx, rnd_coord_t cy)
{
	...
	if (l != NULL) {
		...
		if (data != PCB->Data) {
			...
			pcb_r_delete_entry(PCB_CURRLAYER(PCB)->line_tree, (rnd_box_t *)l);
			pcb_line_free(l);
			pcb_tool_select_by_name(&PCB->hidlib, "buffer");
		}
	}
...
static int get_where(const char *arg, pcb_data_t **data, rnd_coord_t *x, rnd_coord_t *y, rnd_bool *have_coords)
{
	...
	if (end != NULL) {
		...
		*x = pcb_get_value(sx, NULL, NULL, &succ);
		...
		free(tmp);
		if (!succ) {
			rnd_message(PCB_MSG_ERROR, "regpoly(): invalid center coords '%s'\n", dst);
			return -1;
		}
	}
	return a;
...
fgw_error_t pcb_act_regpoly(fgw_arg_t *res, int argc, fgw_arg_t *argv)
{
	...
	if (*end != '\0') {
		rnd_message(PCB_MSG_ERROR, "regpoly(): invalid number of corners '%s'\n", args[a]);
		RND_ACT_IRES(-1);
		return 0;
	}
	a++;
	...
	if (!succ) {
		rnd_message(PCB_MSG_ERROR, "regpoly(): invalid radius '%s'\n", args[a]);
		RND_ACT_IRES(-1);
		return 0;
	}
	a++;
	...
	if (a < argc-1) {
		...
		if (*end != '\0') {
			rnd_message(PCB_MSG_ERROR, "regpoly(): invalid rotation '%s'\n", args[a]);
			RND_ACT_IRES(-1);
			return 0;
		}
	}
...
fgw_error_t pcb_act_roundrect(fgw_arg_t *res, int argc, fgw_arg_t *argv)
{
	...
	if (!succ) {
		rnd_message(PCB_MSG_ERROR, "roundrect(): invalid width or height '%s'\n", args[a]);
		RND_ACT_IRES(-1);
		return 0;
	}
	a++;
	...
	if (a < argc-1) {
		rot = strtod(args[a], &end);
		if (*end != '\0') {
			rnd_message(PCB_MSG_ERROR, "roundrect(): invalid rotation '%s'\n", args[a]);
			RND_ACT_IRES(-1);
			return 0;
		}
		a++;
	...
	if (a < argc-1) {
		...
		for(n = 0, s = args[a]; (n < 4) && (*s != '\0'); s++,n++) {
			switch(*s) {
				...
				default:
					rnd_message(PCB_MSG_ERROR, "roundrect(): invalid corner style character '%c' - has to eb r for round, q for square or c for chamfed", *s);
					RND_ACT_IRES(-1);
					return 0;
			}
		}
	...
	if (a < argc-1) {
		...
		if (*end != '\0') {
			rnd_message(PCB_MSG_ERROR, "roundrect(): roundness '%s'\n", args[a]);
			RND_ACT_IRES(-1);
			return 0;
		}
		if (roundres < 0.1)
...
fgw_error_t pcb_act_circle(fgw_arg_t *res, int argc, fgw_arg_t *argv)
{
	...
	if (!succ) {
		rnd_message(PCB_MSG_ERROR, "circle(): failed to convert dia: invalid coord (%s)\n", args[a]);
		RND_ACT_IRES(1);
		return 0;
	}
