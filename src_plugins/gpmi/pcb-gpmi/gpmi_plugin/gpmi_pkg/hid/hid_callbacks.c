...
void gpmi_hid_set_crosshair(pcb_coord_t x, pcb_coord_t y, int cursor_action)
{
	/* Do nothing */
}

int gpmi_hid_set_layer_group(pcb_layergrp_id_t group, const char *purpose, int purpi, pcb_layer_id_t layer, unsigned int flags, int is_empty, pcb_xform_t **xform)
{
	gpmi_hid_t *h = hid_gpmi_data_get(pcb_exporter);
