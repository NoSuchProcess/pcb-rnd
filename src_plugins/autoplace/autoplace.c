...
#warning subc TODO: when elements are removed, turn this into pcb_subc_t * and remove the fields below
		...
		pcb_r_create_insert_array(rt_s, (const pcb_box_t **) seboxes.array, vtp0_len(&seboxes));
		rt_c = pcb_r_create_tree();
		pcb_r_create_insert_array(rt_c, (const pcb_box_t **) ceboxes.array, vtp0_len(&ceboxes));
		vtp0_uninit(&seboxes);
		vtp0_uninit(&ceboxes);
