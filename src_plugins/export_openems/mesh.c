...
static void mesh_draw_label(pcb_mesh_t *mesh, pcb_mesh_dir_t dir, rnd_coord_t aux, const char *label)
{
	...
	if (dir == PCB_MESH_HORIZONTAL)
		pcb_text_new(mesh->ui_layer_xy, pcb_font_old(PCB, 0, 0), aux, 0, 90, 75, 0, label, pcb_no_flags());
	else
		pcb_text_new(mesh->ui_layer_xy, pcb_font_old(PCB, 0, 0), 0, aux, 0, 75, 0, label, pcb_no_flags());
...
static int mesh_vis_z(pcb_mesh_t *mesh)
{
	...
	for(gid = 0; gid < PCB->LayerGroups.len; gid++) {
		...
		if (grp->ltype & PCB_LYT_COPPER) {
			...
			pcb_text_new(mesh->ui_layer_z, pcb_font_old(PCB, 0, 0), xr+RND_MM_TO_COORD(3), y2 - RND_MM_TO_COORD(1), 0, 100, 0, grp->name, pcb_no_flags());
...
static void ia_load_cb(void *hid_ctx, void *caller_data, rnd_hid_attribute_t *attr)
{
	...
	fname = rnd_hid_fileselect(rnd_gui, "Load mesh settings...",
