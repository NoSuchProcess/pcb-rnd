...
static inline int Vx(pcb_coord_t x)
{
	...
	if (pcbhl_conf.editor.view.flip_x)
		rv = (gport->view.ctx->hidlib->size_x - x - gport->view.x0) / gport->view.coord_per_px + 0.5;
	else
...
static inline int Vy(pcb_coord_t y)
{
	...
	if (pcbhl_conf.editor.view.flip_y)
		rv = (gport->view.ctx->hidlib->size_y - y - gport->view.y0) / gport->view.coord_per_px + 0.5;
	else
...
static inline double Vxd(pcb_coord_t x)
{
	double rv;
	if (pcbhl_conf.editor.view.flip_x)
		rv = (gport->view.ctx->hidlib->size_x - x - gport->view.x0) / gport->view.coord_per_px;
	else
...
static inline double Vyd(pcb_coord_t y)
{
	double rv;
	if (pcbhl_conf.editor.view.flip_y)
		rv = (gport->view.ctx->hidlib->size_y - y - gport->view.y0) / gport->view.coord_per_px;
	else
...
static inline pcb_coord_t Px(int x)
{
	...
	if (pcbhl_conf.editor.view.flip_x)
		rv = gport->view.ctx->hidlib->size_x - (x * gport->view.coord_per_px + gport->view.x0);
	return rv;
}
...
static inline pcb_coord_t Py(int y)
{
	...
	if (pcbhl_conf.editor.view.flip_y)
		rv = gport->view.ctx->hidlib->size_y - (y * gport->view.coord_per_px + gport->view.y0);
	return rv;
}
