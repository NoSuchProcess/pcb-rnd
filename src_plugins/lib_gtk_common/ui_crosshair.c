...
void pcb_gtk_crosshair_set(int x, int y, int action, int offset_x, int offset_y, pcb_gtk_cursor_pos_t *cps, pcb_gtk_view_t *view)
{
	GdkDisplay *display;
	GdkScreen *screen;
	...
	if (view->crosshair_x != x || view->crosshair_y != y) {
		ghid_set_cursor_position_labels(cps, conf_hid_gtk.plugins.hid_gtk.compact_vertical);
		view->crosshair_x = x;
		view->crosshair_y = y;
