...
static lht_node_t *build_polygon(pcb_poly_t *poly)
{
	...
	if ((wrver >= 7) && (poly->enforce_clearance > 0))
		lht_dom_hash_put(obj, build_textf("enforce_clearance", CFMT, poly->enforce_clearance));
	else
...
TODO("subc: for subc-in-subc this should be recursive")
	...
	if (wrver >= 7)
		lht_dom_list_append(doc->root, build_pxms());
...
void *io_lihata_save_as_subd_init(const pcb_plug_io_t *ctx, rnd_hid_dad_subdialog_t *sub, pcb_plug_iot_t type)
{
	...
	if (type & PCB_IOT_PCB) {
		RND_DAD_BEGIN_HBOX(sub->dlg);
			...
			RND_DAD_BEGIN_HBOX(sub->dlg);
				RND_DAD_COMPFLAG(sub->dlg, RND_HATF_TIGHT | RND_HATF_FRAME);
			RND_DAD_END(sub->dlg);

			RND_DAD_BEGIN_VBOX(sub->dlg); /* right column */
				RND_DAD_BEGIN_HBOX(sub->dlg);
