	...
	case PCB_CH_STATE_FIRST:
		{
			...
			if (pcb_crosshair.AttachedObject.Type != PCB_OBJ_VOID) {
				pcb_tool_attach_for_copy(hl, pcb_crosshair_note.X, pcb_crosshair_note.Y, pcb_false);
			}
			break;
		}
...
void pcb_tool_copy_adjust_attached_objects(pcb_hidlib_t *hl)
{
...
pcb_tool_t pcb_tool_copy = {
	"copy", NULL, NULL, 100, NULL, PCB_TOOL_CURSOR_NAMED("crosshair"), 0,
	NULL,
