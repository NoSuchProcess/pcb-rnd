...
{
	rnd_font_t *myfont = pcb_font(PCB, 0, 1);
	rnd_coord_t mWidth = myfont->max_width; /* kicad needs the width of the widest letter */
...
int io_kicad_legacy_write_pcb(pcb_plug_io_t *ctx, FILE *FP, const char *old_filename, const char *new_filename, rnd_bool emergency)
{
	...
	int currentGroup = 0;
	rnd_coord_t outlineThickness = RND_MIL_TO_COORD(10);
