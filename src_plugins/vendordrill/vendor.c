...
fgw_error_t pcb_act_LoadVendorFrom(fgw_arg_t *res, int argc, fgw_arg_t *argv)
{
	...
	if (doc == NULL) {
		rnd_message(PCB_MSG_ERROR, "Could not load vendor resource file \"%s\"\n", fname);
		RND_ACT_IRES(1);
		return 0;
	}
	...
	if (sval == NULL) {
		...
		rnd_message(PCB_MSG_ERROR, "\"%s\" is not a supported units.  Defaulting to inch\n", sval);
		sf = PCB_INCH_TO_COORD(1);
	}
	...
	if (sval != NULL) {
		if (PCB_NSTRCMP(sval, "up") == 0) {
			...
			rnd_message(PCB_MSG_ERROR, "\"%s\" is not a valid rounding type.  Defaulting to up\n", sval);
			rounding_method = ROUND_UP;
		}
	}
...
static void apply_vendor_map(void)
{
	...
	if (n_vendor_drills > 0) {
		...
		rnd_message(PCB_MSG_INFO, "Updated %ld drill sizes out of %ld total\n", (long)changed, (long)tot);
...
TODO(": these 3 loops should be wrapped in a single loop that iterates over attribute keys")
	...
	for (i = 0; i < n_refdes; i++) {
		if ((PCB_NSTRCMP(PCB_UNKNOWN(subc->refdes), ignore_refdes[i]) == 0)
			...
			rnd_message(PCB_MSG_INFO, "Vendor mapping skipped because refdes = %s matches %s\n", PCB_UNKNOWN(subc->refdes), ignore_refdes[i]);
			noskip = 0;
		}
	}
	if (noskip) {
		...
		for (i = 0; i < n_value; i++) {
			if ((PCB_NSTRCMP(PCB_UNKNOWN(vl), ignore_value[i]) == 0)
				...
				rnd_message(PCB_MSG_INFO, "Vendor mapping skipped because value = %s matches %s\n", PCB_UNKNOWN(vl), ignore_value[i]);
				noskip = 0;
			}
		}
	}
	...
	if (noskip && PCB_FLAG_TEST(PCB_FLAG_LOCK, subc)) {
		rnd_message(PCB_MSG_INFO, "Vendor mapping skipped because element %s is locked\n", PCB_UNKNOWN(subc->refdes));
		noskip = 0;
	}
...
static rnd_bool rematch(const char *re, const char *s)
{
	...
	if (re_sei_errno(regex) != 0) {
		rnd_message(PCB_MSG_ERROR, "regexp error: %s\n", re_error_str(re_sei_errno(regex)));
		re_sei_free(regex);
		return pcb_false;
	}
