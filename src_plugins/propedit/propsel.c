...
static void set_net(pcb_propset_ctx_t *st, const char *netname)
{
	...
	if (st->is_attr) {
		...
		st->set_cnt++;
		pcb_ratspatch_append_optimize(st->pcb, RATP_CHANGE_NET_ATTRIB, netname, key, st->s);
		return;
	}
}
...
static long del_net(pcb_propedit_t *ctx, const char *netname, const char *key)
{
	...
	pcb_ratspatch_append_optimize(ctx->pcb, RATP_CHANGE_NET_ATTRIB, netname, key, NULL);
	return 1;
}
