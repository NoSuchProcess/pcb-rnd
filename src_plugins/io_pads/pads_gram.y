...
common_attr_text:
	  T_JUSTIFY T_ID             { pcb_pads_set_justify(ctx, $2); free($2); }
	| T_TEXTSTYLE T_QSTR         { pcb_pads_set_text_style(ctx, $2); free($2); }
...
common_layer:
	T_LAYER T_ID                 { pcb_pads_set_layer(ctx, $2); free($2); }
	;
...
text_style:
	T_TEXTSTYLE T_QSTR             { pcb_pads_text_style_begin(ctx, $2); /* $2 is taken over */ }
		text_style_attrs             { pcb_pads_text_style_end(ctx); }
	;
...
pad_stack:
	T_PADSTACK T_QSTR               { pcb_pads_reset(ctx); pcb_pads_padstack_begin(ctx, $2); /* $2 is taken over */ }
		pstk_attrs nl
		T_SHAPES ':' T_INTEGER nl     { ctx->state.num_shapes = $8; }
		pad_shapes
		T_ENDPADSTACK                 { pcb_pads_padstack_end(ctx); }
	;
...
pad_shape:
	T_PADSHAPE T_QSTR              { pcb_pads_padstack_begin_shape(ctx, $2); free($2); }
		padshape_attrs nl            { pcb_pads_padstack_end_shape(ctx); }
	;
...
/*** Pattern ***/
pattern:
	T_PATTERN T_QSTR nl     { pcb_pads_reset_pattern(ctx); pcb_pads_pattern_begin(ctx, $2); free($2); }
	pattern_chldrn
	T_ENDPATTERN            { pcb_pads_pattern_end(ctx); pcb_pads_reset_pattern(ctx); }
	;
...
/*** Pad ***/
pad:
	T_PAD                                { pcb_pads_pad_begin(ctx); }
		pad_attrs                          { pcb_pads_pad_end(ctx); }
	;
...
pad_attr:
	...
	| T_PADSTYLE T_QSTR                  { pcb_pads_pad_set_style(ctx, $2); free($2); }
...
/*** Poly ***/
poly:
	T_POLY                { pcb_pads_poly_begin(ctx); }
		poly_attrs          { pcb_pads_poly_end(ctx); }
	;
...
poly_attr:
	  ...
	  T_PROPERTY T_QSTR            { pcb_pads_add_property(ctx, (pcb_any_obj_t *)ctx->state.poly, $2); free($2); }
	| coord ',' coord              { pcb_pads_poly_add_vertex(ctx, XCRD($1), YCRD($3)); }
	| common_origin
	| common_layer
	| common_width
	;
...
/*** Line ***/
line:
	T_LINE                        { pcb_pads_reset(ctx); }
		line_attrs                  { pcb_pads_add_line(ctx); pcb_pads_reset(ctx); }
	;
...
/*** Attribute ***/
attribute:
	T_ATTRIBUTE                         { pcb_pads_reset(ctx); ctx->state.is_text = 0; }
		attribute_attrs                   { pcb_pads_add_text(ctx); pcb_pads_reset(ctx); }
	;
...
attribute_attr:
	  T_ATTR T_QSTR T_QSTR      { pcb_pads_set_attr_val(ctx, $2, $3); /* $2 and $3 are taken over */ }
	| common_attr_text
	| common_origin
	| common_layer
	;
...
/*** Arc ***/
arc:
	T_ARC                   { pcb_pads_reset(ctx); }
		arc_attrs             { pcb_pads_add_arc(ctx); pcb_pads_reset(ctx); }
	;
...
/*** Text ***/
text:
	T_TEXT                        { pcb_pads_reset(ctx); ctx->state.is_text = 1; }
		text_attrs                  { pcb_pads_add_text(ctx); pcb_pads_reset(ctx); }
	;
...
text_attr:
	  T_TEXT T_QSTR               { pcb_pads_set_text_str(ctx, $2); /* $2 is taken over */ }
