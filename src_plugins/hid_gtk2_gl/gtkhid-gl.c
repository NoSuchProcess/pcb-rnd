...
void ghid_gl_use_mask(pcb_mask_op_t use_it)
{
	...
	case HID_MASK_SET:
		/* Write '1' to the stencil buffer where the solder-mask should not be drawn ("transparent", "cutout"). */
	...
	case HID_MASK_CLEAR:
		/* Write '0' to the stencil buffer where the solder-mask should not be drawn. */
		glStencilFunc(GL_ALWAYS, stencil_bit, stencil_bit);	/* Always pass stencil test, write stencil_bit */
		glStencilMask(stencil_bit);	/* Only write to our subcompositing stencil bitplane */
