...
static void sym_flush(rnd_design_t *hl, symattr_t *sattr)
{
...
typedef struct {
	...
	rnd_coord_t minimum_comp_pin_drill;
	int trax_version;
...
static int rdax_pad(read_state_t *st, FILE *FP, pcb_subc_t *subc, int component)
{
	...
	int valid = 1;
	int success;
	rnd_coord_t X, Y, X_size, Y_size, Thickness, Clearance, Mask, Drill;
	pcb_data_t *data = (subc == NULL) ? st->pcb->Data : subc->data;
	pcb_pstk_t *ps;
   ...
   shape:
	...
	{ /* pad not within element, i.e. a free pad/pin/via */
		...
		switch(Shape) {
			case 1: /* round */
				for(n = 0; n < 7; n++) {
					rnd_coord_t clr = (sh[n].layer_mask & PCB_LYT_MASK) ? Clearance : 0;
...
static int rdax_fill(read_state_t *st, FILE *FP, pcb_subc_t *subc)
{
	...
	rnd_coord_t X1, Y1, X2, Y2, Clearance;
	pcb_layer_t *ly;
...
static int autotrax_create_layers(read_state_t *st)
{
	...
	st->protel_to_stackup[2] = pcb_layer_create(st->pcb, g - st->pcb->LayerGroups.grp, "Mid1", 0);

	g = pcb_get_grp_new_intern(st->pcb, -1);
	st->protel_to_stackup[3] = pcb_layer_create(st->pcb, g - st->pcb->LayerGroups.grp, "Mid2", 0);

	g = pcb_get_grp_new_intern(st->pcb, -1);
	st->protel_to_stackup[4] = pcb_layer_create(st->pcb, g - st->pcb->LayerGroups.grp, "Mid3", 0);

	g = pcb_get_grp_new_intern(st->pcb, -1);
	st->protel_to_stackup[5] = pcb_layer_create(st->pcb, g - st->pcb->LayerGroups.grp, "Mid4", 0);

	g = pcb_get_grp_new_intern(st->pcb, -1);
	st->protel_to_stackup[9] = pcb_layer_create(st->pcb, g - st->pcb->LayerGroups.grp, "GND", 0);
...
TODO("this does not handle return -1")
	...
	rnd_rtree_insert(st->pcb->Data->subc_tree, new_module, (rnd_rtree_box_t *)new_module);
...
int io_autotrax_read_pcb(pcb_plug_io_t *ctx, pcb_board_t *Ptr, const char *Filename, rnd_conf_role_t settings_dest)
{
	...
	if (box != NULL) {
		Ptr->hidlib.dwg.X2 = box->X2;
		Ptr->hidlib.dwg.Y2 = box->Y2;
	}
	else
