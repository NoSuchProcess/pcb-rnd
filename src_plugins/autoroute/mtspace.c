...
static pcb_r_dir_t mts_remove_one(const rnd_rnd_box_t * b, void *cl)
{
	...
	if (b->X1 == info->box.X1 && b->X2 == info->box.X2 &&
		...
		pcb_r_delete_entry_free_data(info->tree, (rnd_rnd_box_t *)b, free);
		longjmp(info->env, 1);
	}
...
void mtspace_add(mtspace_t * mtspace, const rnd_rnd_box_t * box, mtspace_type_t which, rnd_coord_t clearance)
{
	...
	pcb_r_insert_entry(which_tree(mtspace, which), (const rnd_rnd_box_t *) filler);
}
...
static inline void heap_append(pcb_heap_t * heap, rnd_cheap_point_t * desired, rnd_rnd_box_t * newone)
{
 ...
 */
static pcb_r_dir_t query_one(const rnd_rnd_box_t * box, void *cl)
{
...
static void qloop(struct query_closure *qc, rnd_rtree_t * tree, heap_or_vector res, rnd_bool is_vec)
{
	...
	while (!(qc->desired ? pcb_heap_is_empty(qc->checking.h) : vector_is_empty(qc->checking.v))) {
		cbox = qc->desired ? (rnd_rnd_box_t *) pcb_heap_remove_smallest(qc->checking.h) : (rnd_rnd_box_t *) vector_remove_last(qc->checking.v);
		if (setjmp(qc->env) == 0) {
