...
typedef struct routedata_s {
	...
	rnd_rtree_t *layergrouptree[PCB_MAX_LAYERGRP];	/* no silkscreen layers here =) */
	...
	rnd_coord_t max_bloat;
	rnd_coord_t max_keep;
	mtspace_t *mtspace;
} routedata_t;
...
static rnd_box_t edge_to_box(const routebox_t * rb, rnd_direction_t expand_dir);
...
int no_planes(const rnd_box_t * b, void *cl)
{
...
static void init_const_box(routebox_t * rb, rnd_coord_t X1, rnd_coord_t Y1, rnd_coord_t X2, rnd_coord_t Y2, rnd_coord_t clearance)
{
	rnd_box_t *bp = (rnd_box_t *) & rb->box;	/* note discarding const! */
	assert(!rb->flags.inited);
	assert(X1 <= X2 && Y1 <= Y2);
	bp->X1 = X1 - clearance;
	bp->Y1 = Y1 - clearance;
...
static rnd_rtree_dir_t __found_one_on_lg(void *cl, void *obj, const rnd_rtree_box_t *box)
{
	struct rb_info *inf = (struct rb_info *) cl;
	routebox_t *rb = (routebox_t *)obj;
	rnd_box_t sb;
	...
	inf->winner = rb;
	if (rb->type == PLANE)
		return rnd_RTREE_DIR_FOUND_CONT; /* keep looking for something smaller if a plane was found */
	longjmp(inf->env, 1);
	return rnd_RTREE_DIR_NOT_FOUND_CONT;
}
...
static routebox_t *FindRouteBoxOnLayerGroup(routedata_t * rd, rnd_coord_t X, rnd_coord_t Y, rnd_cardinal_t layergroup)
{
	...
	if (setjmp(info.env) == 0)
		rnd_rtree_search_any(rd->layergrouptree[layergroup], (rnd_rtree_box_t *)&info.query, NULL, __found_one_on_lg, &info, NULL);
	return info.winner;
}
	...
	for (group = 0; group < pcb_max_group(PCB); group++) {
		for (i = 0; i < PCB->LayerGroups.grp[group].len; i++) {
			...
			if ((pcb_layer_flags(PCB, lid) & PCB_LYT_COPPER) && PCB->Data->Layer[lid].meta.real.vis) {
				routing_layers++;
				is_layer_group_active[group] = rnd_true;
				break;
			}
			else
				is_layer_group_active[group] = rnd_false;
		}
	}
...
/*	rd->layergrouptree = calloc(sizeof(rd->layergrouptree[0]), rd->max_layers);*/
	...
	usedGroup[front] = rnd_true;
	usedGroup[back] = rnd_true;
 ...
 */
static rnd_heap_cost_t pcb_cost_to_layerless_box(const rnd_cheap_point_t * p, rnd_cardinal_t point_layer, const rnd_box_t * b)
{
...
rnd_bool TargetPoint(rnd_cheap_point_t * nextpoint, const routebox_t * target)
{
/*	if (target->type == PIN) {
...
static rnd_box_t bloat_routebox(routebox_t * rb)
{
	rnd_box_t r;
	...
	r = rnd_bloat_box(&rb->sbox, clearance + HALF_THICK(AutoRouteParameters.style->Thick));
	return r;
}
...
static void showbox(rnd_box_t b, pcb_dimension_t thickness, int group)
{
	pcb_line_t *line;
...
static vector_t *path_conflicts(routebox_t * rb, routebox_t * conflictor, rnd_bool branch)
{
	if (branch)
...
static rnd_rtree_dir_t __region_within_guess(void *cl, void *obj, const rnd_rtree_box_t *box)
{
	const rnd_box_t *region = (const rnd_box_t *)box;
	...
	return (pcb_cost_to_region < mtc->nearest_cost) ? rnd_RTREE_DIR_FOUND_CONT : rnd_RTREE_DIR_NOT_FOUND_CONT;
}

static rnd_rtree_dir_t __found_new_guess(void *cl, void *obj, const rnd_rtree_box_t *box)
{
	struct minpcb_cost_target_closure *mtc = (struct minpcb_cost_target_closure *) cl;
	routebox_t *guess = (routebox_t *)obj;
	...
	if (pcb_cost_to_guess < mtc->nearest_cost) {
		...
		return rnd_RTREE_DIR_NOT_FOUND_CONT; /* not less expensive than our last guess */
}
...
static routebox_t *minpcb_cost_target_to_point(const rnd_cheap_point_t * CostPoint,
																					 rnd_cardinal_t CostPointLayer, rnd_rtree_t * targets, routebox_t * target_guess)
{
	...
	rnd_rtree_search_any(targets, NULL, __region_within_guess, __found_new_guess, &mtc, NULL);
													...
													rnd_heap_cost_t pcb_cost_to_point, routebox_t * minpcb_cost_target_guess, rnd_direction_t expand_dir, rnd_rtree_t * targets)
{
	...
	assert(rb->type == PLANE || rb->conflicts_with != NULL || rb->flags.is_via
				 ...
				 || ((expand_dir == RND_NORTH || expand_dir == RND_SOUTH) ? rb->sbox.X1 <=
						 CostPointX && CostPointX < rb->sbox.X2 && CostPointY == (expand_dir == RND_NORTH ? rb->sbox.Y1 : rb->sbox.Y2 - 1) :
						 /* expand_dir==EAST || expand_dir==WEST */
						 rb->sbox.Y1 <= CostPointY && CostPointY < rb->sbox.Y2 &&
						 CostPointX == (expand_dir == RND_EAST ? rb->sbox.X2 - 1 : rb->sbox.X1)));
#endif
...
static edge_t *CreateEdge2(routebox_t * rb, rnd_direction_t expand_dir,
													 edge_t * previous_edge, rnd_rtree_t * targets, routebox_t * guess)
{
	rnd_box_t thisbox;
...
static edge_t *CreateViaEdge(const rnd_box_t * area, rnd_cardinal_t group,
														 ...
														 conflict_t to_site_conflict, conflict_t through_site_conflict, rnd_rtree_t * targets)
{
	routebox_t *rb;
	...
	rb = CreateExpansionArea(area, group, parent, rnd_true, previous_edge);
	rb->flags.is_via = 1;
	rb->came_from = RND_ANY_DIR;
 ...
 */
static edge_t *CreateEdgeWithConflicts(const rnd_box_t * interior_edge,
																			 ...
																			 rnd_heap_cost_t cost_penalty_to_box, rnd_rtree_t * targets)
{
	...
	rb = CreateExpansionArea(interior_edge, previous_edge->rb->group, previous_edge->rb, rnd_true, previous_edge);
	path_conflicts(rb, container, rnd_true);	/* crucial! */
...
static rnd_box_t edge_to_box(const routebox_t * rb, rnd_direction_t expand_dir)
{
	rnd_box_t b = shrink_routebox(rb);
...
struct broken_boxes {
	rnd_box_t left, center, right;
	rnd_bool is_valid_left, is_valid_center, is_valid_right;
};

static struct broken_boxes break_box_edge(const rnd_box_t * original, rnd_direction_t which_edge, routebox_t * breaker)
{
	rnd_box_t origbox, breakbox;
...
#ifndef NDEBUG
static int share_edge(const rnd_box_t * child, const rnd_box_t * parent)
{
	return
...
static int edge_intersect(const rnd_box_t * child, const rnd_box_t * parent)
{
																			 ...
																			 routebox_t * parent, rnd_bool relax_edge_requirements, edge_t * src_edge)
{
...
#ifndef NDEBUG
	{
		...
		rnd_box_t b1 = rb->sbox, b2 = parent->sbox;
...
struct E_result {
	routebox_t *parent;
	routebox_t *n, *e, *s, *w;
	rnd_coord_t keep, bloat;
	rnd_box_t inflated, orig;
	int done;
};
 ...
 */
static rnd_rtree_dir_t __Expand_this_rect(void *cl, void *obj, const rnd_rtree_box_t *box)
{
	struct E_result *res = (struct E_result *) cl;
	routebox_t *rb = (routebox_t *) box;
	rnd_box_t rbox;
	rnd_coord_t dn, de, ds, dw, bloat;
	...
	if (rb->flags.nobloat) {
		...
		if (rbox.X2 <= res->inflated.X1 + bloat ||
			...
			return rnd_RTREE_DIR_NOT_FOUND_CONT; /* doesn't touch */
	}
	else {
	...
	if (rb == res->parent || rb->parent.expansion_area == res->parent)
		return rnd_RTREE_DIR_NOT_FOUND_CONT; /* don't see what we came from */
...
static rnd_bool boink_box(routebox_t * rb, struct E_result *res, rnd_direction_t dir)
{
	rnd_coord_t bloat;
	if (rb->style->Clearance > res->keep)
 ...
 */
struct E_result *Expand(rnd_rtree_t * rtree, edge_t * e, const rnd_box_t * box)
{
	...
	switch (e->expand_dir) {
	case RND_ANY_DIR:
		ans.inflated.X1 = (e->rb->came_from == RND_EAST ? ans.orig.X1 : PCB->hidlib.dwg.X1);
		ans.inflated.Y1 = (e->rb->came_from == RND_SOUTH ? ans.orig.Y1 : PCB->hidlib.dwg.Y1);
		ans.inflated.X2 = (e->rb->came_from == RND_WEST ? ans.orig.X2 : PCB->hidlib.dwg.X2);
		ans.inflated.Y2 = (e->rb->came_from == RND_NORTH ? ans.orig.Y2 : PCB->hidlib.dwg.Y2);
	...
	case RND_NE:
		...
		noshrink = 0;
		ans.inflated.X1 = box->X1 - ans.bloat;
		ans.inflated.X2 = PCB->hidlib.dwg.X2;
	...
	case RND_SE:
		...
		ans.inflated.X1 = box->X1 - ans.bloat;
		ans.inflated.X2 = PCB->hidlib.dwg.X2;
		ans.inflated.Y2 = PCB->hidlib.dwg.Y2;
		ans.inflated.Y1 = box->Y1 - ans.bloat;
		break;
	case RND_SOUTH:
	...
	rnd_rtree_search_any(rtree, (rnd_rtree_box_t *)&ans.inflated, NULL, __Expand_this_rect, &ans, NULL);
 ...
 */
static int blocker_to_heap(rnd_heap_t * heap, routebox_t * rb, rnd_box_t * box, rnd_direction_t dir)
{
	rnd_box_t b = rb->sbox;
 ...
 */
static routebox_t *CreateBridge(const rnd_box_t * area, routebox_t * parent, rnd_direction_t dir)
{
	routebox_t *rb = (routebox_t *) malloc(sizeof(*rb));
	memset((void *) rb, 0, sizeof(*rb));
	assert(area && parent);
 ...
 */
void
moveable_edge(vector_t * result, const rnd_box_t * box, rnd_direction_t dir,
							routebox_t * rb,
							routebox_t * blocker, edge_t * e, rnd_rtree_t * targets,
							routeone_state_t *s, rnd_rtree_t * tree, vector_t * area_vec)
{
	rnd_box_t b;
	...
	case RND_EAST:
		b.X1 = b.X2;
		b.X2++;
		if (b.X2 >= PCB->hidlib.dwg.Y2 - AutoRouteParameters.bloat)
			return;										/* off board edge */
		break;
	case RND_SOUTH:
		b.Y1 = b.Y2;
		b.Y2++;
		if (b.Y2 >= PCB->hidlib.dwg.Y2 - AutoRouteParameters.bloat)
			return;										/* off board edge */
		break;
	case RND_WEST:
		b.X2 = b.X1;
		...
		break;
	case RND_NE:
		if (b.Y1 <= AutoRouteParameters.bloat + 1 && b.X2 >= PCB->hidlib.dwg.X2 - AutoRouteParameters.bloat - 1)
			return;										/* off board edge */
		if (b.Y1 <= AutoRouteParameters.bloat + 1)
			dir = RND_EAST;								/* north off board edge */
		if (b.X2 >= PCB->hidlib.dwg.X2 - AutoRouteParameters.bloat - 1)
			dir = RND_NORTH;							/* east off board edge */
		break;
	case RND_SE:
		if (b.Y2 >= PCB->hidlib.dwg.Y2 - AutoRouteParameters.bloat - 1 && b.X2 >= PCB->hidlib.dwg.X2 - AutoRouteParameters.bloat - 1)
			return;										/* off board edge */
		if (b.Y2 >= PCB->hidlib.dwg.Y2 - AutoRouteParameters.bloat - 1)
			dir = RND_EAST;								/* south off board edge */
		if (b.X2 >= PCB->hidlib.dwg.X2 - AutoRouteParameters.bloat - 1)
			dir = RND_SOUTH;							/* east off board edge */
		break;
	case RND_SW:
		if (b.Y2 >= PCB->hidlib.dwg.Y2 - AutoRouteParameters.bloat - 1 && b.X1 <= AutoRouteParameters.bloat + 1)
			return;										/* off board edge */
		if (b.Y2 >= PCB->hidlib.dwg.Y2 - AutoRouteParameters.bloat - 1)
...
struct break_info {
	...
	rnd_box_t box;
	rnd_direction_t dir;
	rnd_bool ignore_source;
};

static rnd_rtree_dir_t __GatherBlockers(void *cl, void *obj, const rnd_rtree_box_t *box)
{
	routebox_t *rb = (routebox_t *) box;
 ...
 */
vector_t *BreakManyEdges(routeone_state_t * s, rnd_rtree_t * targets, rnd_rtree_t * tree,
												 vector_t * area_vec, struct E_result * ans, routebox_t * rb, edge_t * e)
{
	...
	if (e->expand_dir == RND_NE || e->expand_dir == RND_SE || e->expand_dir == RND_SW || e->expand_dir == RND_NW) {
		rnd_box_t *fb = (rnd_box_t *) & fake.sbox;
	...
	for (dir = RND_NORTH; dir <= RND_WEST; dir = directionIncrement(dir)) {
		...
		if (e->expand_dir != ((dir + 2) % 4)) {
			...
			case RND_NORTH:
				...
				rnd_rtree_search_any(tree, (rnd_rtree_box_t *)&bi.box, NULL, __GatherBlockers, &bi, &tmp);
				rb->n = tmp;
				break;
			case RND_EAST:
	...
	for (dir = RND_NORTH; dir <= RND_WEST; dir = directionIncrement(dir)) {
		if (heap[dir] && !rnd_heap_is_empty(heap[dir])) {
			...
			if (broke.is_valid_left) {
				 ...
				 * which it belongs to.
				 */
				rnd_box_t db = previous_edge(last, dir, &rb->sbox);
...
static rnd_rtree_dir_t foib_rect_in_reg(void *cl, void *obj, const rnd_rtree_box_t *box)
{
...
static routebox_t *FindOneInBox(rnd_rtree_t * rtree, routebox_t * rb)
{
	...
	if (setjmp(foib.env) == 0)
		rnd_rtree_search_any(rtree, (rnd_rtree_box_t *)&r, NULL, foib_rect_in_reg, &foib, NULL);
...
static rnd_rtree_dir_t ftherm_rect_in_reg(void *cl, void *obj, const rnd_rtree_box_t *box)
{
...
routebox_t *FindThermable(rnd_rtree_t * rtree, routebox_t * rb)
{
	...
	if (setjmp(info.env) == 0) {
		rnd_rtree_search_any(rtree, (rnd_rtree_box_t *)&info.query, NULL, ftherm_rect_in_reg, &info, NULL);
		return NULL;
	}
	return info.plane;
}
...
static void RD_DrawThermal(routedata_t * rd, rnd_coord_t X, rnd_coord_t Y, rnd_cardinal_t group, rnd_cardinal_t layer, routebox_t * subnet, rnd_bool is_bad)
{
	...
	rnd_rtree_insert(rd->layergrouptree[rb->group], &rb->box, (rnd_rtree_box_t *)&rb->box);
	rb->flags.homeless = 0;
}
						...
						rnd_coord_t Y2, rnd_coord_t halfthick, rnd_cardinal_t group, routebox_t * subnet, rnd_bool is_bad, rnd_bool is_45)
{
	...
	routebox_t *rb;
	rnd_coord_t ka = AutoRouteParameters.style->Clearance;
										 ...
										 rnd_coord_t halfthick, rnd_cardinal_t group, routebox_t * subnet, rnd_bool is_bad, rnd_bool last_was_x)
{
	...
	if (end.X == start.X) {
		RD_DrawLine(rd, start.X, start.Y, end.X, end.Y, halfthick, group, subnet, is_bad, rnd_false);
		return rnd_false;
	}
	...
	assert(AutoRouteParameters.is_smoothing || rnd_point_in_closed_box(box1, knee.X, knee.Y)
				 || rnd_point_in_closed_box(box2, knee.X, knee.Y));

	if (1 || !AutoRouteParameters.is_smoothing) {
		/* draw standard manhattan paths */
		RD_DrawLine(rd, start.X, start.Y, knee.X, knee.Y, halfthick, group, subnet, is_bad, rnd_false);
		RD_DrawLine(rd, knee.X, knee.Y, end.X, end.Y, halfthick, group, subnet, is_bad, rnd_false);
	}
	else {
		...
		RD_DrawLine(rd, start.X, start.Y, kneestart.X, kneestart.Y, halfthick, group, subnet, is_bad, rnd_false);
		RD_DrawLine(rd, kneestart.X, kneestart.Y, kneeend.X, kneeend.Y, halfthick, group, subnet, is_bad, rnd_true);
		RD_DrawLine(rd, kneeend.X, kneeend.Y, end.X, end.Y, halfthick, group, subnet, is_bad, rnd_false);
	}
	return (knee.X != end.X);
}
...
#if 0
static void add_clearance(rnd_cheap_point_t * nextpoint, const rnd_box_t * b)
{
	if (nextpoint->X == b->X1) {
...
static void TracePath(routedata_t * rd, routebox_t * path, const routebox_t * target, routebox_t * subnet, rnd_bool is_bad)
{
	...
	if (path->flags.is_thermal) {
		...
		if (path->flags.circular)
			b = rnd_shrink_box(&b, MIN(b.X2 - b.X1, b.Y2 - b.Y1) / 5);
		nextpoint = rnd_closest_cheap_point_in_box(&lastpoint, &b);
		if (AutoRouteParameters.last_smooth)
			RD_DrawLine(rd, lastpoint.X, lastpoint.Y, nextpoint.X, nextpoint.Y, halfwidth, path->group, subnet, is_bad, rnd_true);
		else
	...
	RD_DrawLine(rd, -1, 0, 0, 0, 0, 0, NULL, rnd_false, rnd_false);
...
CreateSearchEdge(routeone_state_t *s, vetting_t * work, edge_t * parent,
								 routebox_t * rb, conflict_t conflict, rnd_rtree_t * targets, rnd_bool in_plane)
{
	routebox_t *target;
	rnd_box_t b;
	rnd_heap_cost_t cost;
...
add_via_sites(routeone_state_t *s,
							...
							conflict_t within_conflict_level, edge_t * parent_edge, rnd_rtree_t * targets, rnd_coord_t shrink, rnd_bool in_plane)
{
...
do_via_search(edge_t * search, routeone_state_t *s,
							struct routeone_via_site_state *vss, mtspace_t * mtspace, rnd_rtree_t * targets)
{
	int i, j, count = 0;
	rnd_coord_t radius, clearance;
	vetting_t *work;
	...
	for (i = 0; i < 3; i++) {
		...
		while (!vector_is_empty(v)) {
			rnd_box_t cliparea;
			rnd_box_t *area = (rnd_box_t *) vector_remove_last(v);
...
static rnd_bool net_id(routebox_t * rb, long int id)
{
	routebox_t *p;
	LIST_LOOP(rb, same_net, p);
...
static rnd_rtree_dir_t __conflict_source(void *cl, void *obj, const rnd_rtree_box_t *box)
{
	routebox_t *rb = (routebox_t *) box;
	if (rb->flags.touched || rb->flags.fixed)
		...
		path_conflicts(dis, rb, rnd_false);
		touch_conflicts(dis->conflicts_with, 1);
	}
	return rnd_RTREE_DIR_FOUND_CONT;
}

static void source_conflicts(rnd_rtree_t * tree, routebox_t * rb)
{
	if (!AutoRouteParameters.with_conflicts)
		return;
	rnd_rtree_search_any(tree, (rnd_rtree_box_t *)&rb->sbox, NULL, __conflict_source, rb, NULL);
...
static routeone_status_t RouteOne(routedata_t * rd, routebox_t * from, routebox_t * to, int max_edges)
{
	...
	while (!rnd_heap_is_empty(s.workheap)) {
		...
		if (e->rb->type == PLANE) {
			...
			if (pin) {
				rnd_box_t b = shrink_routebox(pin);
				edge_t *ne;
				routebox_t *nrb;
				assert(pin->flags.target);
				nrb = CreateExpansionArea(&b, e->rb->group, e->rb, rnd_true, e);
		...
		if (e->flags.is_via) {			/* special case via */
			...
			if (intersecting && intersecting->flags.target && intersecting->type == PLANE) {
				...
				edge_t *ne;
				routebox_t *nrb;
				rnd_box_t b = shrink_routebox(e->rb);
				 ...
				 */
				rnd_rtree_insert(rd->layergrouptree[e->rb->group], &e->rb->box, (rnd_rtree_box_t *)&e->rb->box);
				...
				goto dontexpand;
																   ...
																   collisions. */
				rnd_box_t a = bloat_routebox(intersecting), b;
				edge_t *ne;
				int i, j;
		...
		if (1) {
			...
			if (!rnd_box_is_good(&ans->inflated))
				goto dontexpand;
			nrb = CreateExpansionArea(&ans->inflated, e->rb->group, e->rb, rnd_true, e);
			rnd_rtree_insert(rd->layergrouptree[nrb->group], &nrb->box, (rnd_rtree_box_t *)&nrb->box);
			vector_append(area_vec, nrb);
			nrb->flags.homeless = 0;	/* not homeless any more */
			...
			if (AutoRouteParameters.use_vias && !e->rb->flags.is_via && e->cost + AutoRouteParameters.ViaCost < s.best_cost)
				add_via_sites(&s, &vss, rd->mtspace, nrb, NO_CONFLICT, e, targets, 0, rnd_false);
			goto dontexpand;
		}
	dontexpand:
		DestroyEdge(&e);
...
static void InitAutoRouteParameters(int pass, pcb_route_style_t * style, rnd_bool with_conflicts, rnd_bool is_smoothing, rnd_bool lastpass)
{
	int i;
	/* routing style */
...
#ifndef NDEBUG
rnd_rtree_dir_t bad_boy(void *cl, void *obj, const rnd_rtree_box_t *box_)
{
	routebox_t *box = (routebox_t *)box_;
	if (box->type == EXPANSION_AREA)
		return rnd_RTREE_DIR_FOUND_CONT;
	return rnd_RTREE_DIR_NOT_FOUND_CONT;
}

rnd_bool no_expansion_boxes(routedata_t * rd)
{
	int i;
	rnd_box_t big;
	big.X1 = 0;
	...
	for (i = 0; i < pcb_max_group(PCB); i++) {
		if (rnd_rtree_search_any(rd->layergrouptree[i], (rnd_rtree_box_t *)&big, NULL, bad_boy, NULL, NULL))
			return rnd_false;
	}
	return rnd_true;
}
...
static rnd_rtree_dir_t ripout_livedraw_obj_cb(void *cl, void *obj, const rnd_rtree_box_t *box_)
{
	routebox_t *box = (routebox_t *)box_;
...
struct routeall_status RouteAll(routedata_t * rd)
{
	...
	{
		double area;
		rnd_box_t bb = shrink_routebox(net);
		LIST_LOOP(net, same_net, p);
		{
...
static rnd_rtree_dir_t fpstk_rect(void *cl, void *obj, const rnd_rtree_box_t *box)
{
...
static int FindPin(const rnd_box_t *box, pcb_pstk_t **ps_out)
{
	...
	info.y = box->Y1;
	if (setjmp(info.env) == 0) {
		rnd_rtree_search_any(PCB->Data->padstack_tree, (rnd_rtree_box_t *)box, NULL, fpstk_rect, &info, NULL);
	}
	else {
		*ps_out = info.ps;
		return PCB_OBJ_PSTK;
...
/* returns 'rnd_true' if any paths were added. */
rnd_bool IronDownAllUnfixedPaths(routedata_t * rd)
{
	...
	LIST_LOOP(rd->first_net, different_net, net);
	{
		LIST_LOOP(net, same_net, p);
		{
			if (!p->flags.fixed) {
				...
				if (p->type == LINE) {
					rnd_coord_t halfwidth = HALF_THICK(p->style->Thick);
		...
		LIST_LOOP(net, same_net, p);
		{
			if (p->type == THERMAL) {
				...
				if (pin) {
					pcb_undo_add_obj_to_clear_poly(type, pin->parent.data, pin, pin, rnd_false);
					...
					pcb_undo_add_obj_to_clear_poly(type, pin->parent.data, pin, pin, rnd_true);
...
rnd_bool AutoRoute(rnd_bool selected)
{
	rnd_bool changed = rnd_false;
	routedata_t *rd;
	int i;
...
*/
				...
				InitAutoRouteParameters(0, a->style, rnd_false, rnd_true, rnd_true);
...
donerouting:
	...
	if (conf_core.editor.live_routing) {
		int i;
		rnd_box_t big = { 0, 0, RND_MAX_COORD, RND_MAX_COORD };
		for (i = 0; i < pcb_max_group(PCB); i++) {
			rnd_rtree_search_any(rd->layergrouptree[i], (rnd_rtree_box_t *)&big, NULL, ripout_livedraw_obj_cb, NULL, NULL);
		}
	}
#ifdef ROUTE_DEBUG
	if (ddraw != NULL)
