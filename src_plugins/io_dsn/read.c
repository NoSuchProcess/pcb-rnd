...
static rnd_coord_t COORD(dsn_read_t *ctx, gsxl_node_t *n)
{
	char *end, *s = STRE(n);
...
/* load coordinates from nodes starting from src, hoppin on ->next, into
   rnd_coord_t dst[]. Each coord is converted according to the next char
...
static rnd_coord_t dsn_load_aper(dsn_read_t *ctx, gsxl_node_t *c)
{
	rnd_coord_t res = COORD(ctx, c);
	if (res == 0)
		res = 1;
	return res;
}
...
static void boundary_line(pcb_layer_t *oly, rnd_coord_t x1, rnd_coord_t y1, rnd_coord_t x2, rnd_coord_t y2, rnd_coord_t aper)
{
	if (aper <= 0)
		aper = 1;
...
static int qarc_angle(rnd_coord_t cx, rnd_coord_t cy, rnd_coord_t x, rnd_coord_t y, rnd_coord_t *r)
{
	rnd_coord_t dx = x - cx;
	rnd_coord_t dy = y - cy;
...
static int dsn_parse_wire_qarc(dsn_read_t *ctx, gsxl_node_t *wrr, pcb_subc_t *subc, pcb_layer_t *force_ly)
{
	...
	pcb_layer_t *ly;
	rnd_coord_t r1, r2, aper;
	rnd_coord_t crd[6]; /* sx, sy, ex, ey, cx, cy */
	int sa, ea;
...
{
	rnd_actionva(&ctx->pcb->hidlib, "Netlist", "Freeze", NULL);
	rnd_actionva(&ctx->pcb->hidlib, "Netlist", "Clear", NULL);
	...
	rnd_actionva(&ctx->pcb->hidlib, "Netlist", "Sort", NULL);
	rnd_actionva(&ctx->pcb->hidlib, "Netlist", "Thaw", NULL);
